    ...
    return (
      <div className="max-w-7xl mx-auto">
        <div className="animate-pulse space-y-4">
          <div className="h-8 bg-slate-800/60 rounded w-1/4"></div>
          <div className="h-64 bg-slate-800/60 rounded"></div>
        </div>
      </div>
    )
  }
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold text-slate-100">{review.file_name}</h1>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-slate-800/60 border border-slate-700/50 hover:bg-slate-800 hover:border-slate-600/60 rounded-lg text-sm font-medium text-slate-300 transition-colors"
        >
      ...
      {structured ? (
        <>
            ...
            <div className="lg:col-span-1 bg-gradient-to-br from-cyan-500/10 via-blue-500/10 to-transparent rounded-xl p-6 border border-cyan-500/20">
              <div className="flex flex-col items-center text-center">
                <ScoreBadge 
                  ...
                  size="lg"
                />
                <h2 className="text-lg font-semibold text-slate-100 mt-4">Overall Quality</h2>
                <p className="text-sm text-slate-500">Code quality assessment</p>
              </div>
            </div>

            {/* Issues Summary */}
            <div className="lg:col-span-2 bg-[#2e333d] rounded-xl p-6 border border-slate-700/50">
              <h2 className="text-xl font-bold text-slate-100 mb-4">Issues Summary</h2>
              ...
              <div className="mt-4 text-center text-sm text-slate-500">
                Total: <span className="font-bold text-slate-100">{structured.summary.totalIssues} issues found</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#2e333d] rounded-xl p-6 border border-slate-700/50">
            <h2 className="text-xl font-bold text-slate-100 mb-6">Code Metrics</h2>
          ...
          {structured.issues.length > 0 && (
            <div className="bg-[#2e333d] rounded-xl p-6 border border-slate-700/50">
              <div className="mb-6">
                <h2 className="text-xl font-bold text-slate-100 mb-4">Issues Found</h2>
                
                {/* Filter Controls */}
                <div className="space-y-3">
                  {/* Severity Filter - Buttons */}
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm font-medium text-slate-400 mr-2">Severity:</span>
                    <button
                  ...
                  <div className="flex items-center gap-2">
                    <label htmlFor="category-filter" className="text-sm font-medium text-slate-400">
                      Category:
                    </label>
                    <select
                      id="category-filter"
                      ...
                      className="px-4 py-2 bg-slate-900/60 border border-slate-700/50 rounded-lg text-sm font-medium text-slate-200 hover:bg-slate-800/60 focus:outline-none focus:ring-2 focus:ring-cyan-500/30 focus:border-cyan-500/50 transition-colors cursor-pointer"
                    >
              ...
              <div className="space-y-4">
                {filteredIssues.length === 0 ? (
                  <p className="text-center text-slate-500 py-8">No issues in this category</p>
                ) : (
              ...
              <ul className="space-y-2">
                {structured.recommendations.map((rec, idx) => (
                  <li key={idx} className="flex items-start gap-3">
                    ...
                    <span className="text-slate-300 flex-1">{rec}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="bg-[#2e333d] rounded-xl p-6 border border-slate-700/50">
          <h2 className="text-xl font-bold text-slate-100 mb-4">Review Report</h2>
      ...
      {structured && review.report_md && (
        <details className="bg-slate-900/40 rounded-lg border border-slate-800/60">
          <summary className="px-4 py-3 cursor-pointer font-medium text-slate-400 hover:bg-slate-800/40">
