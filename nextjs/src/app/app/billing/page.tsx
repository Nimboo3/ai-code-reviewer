...
export default function BillingPage() {
  ...
  return (
    <div className="min-h-screen p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-100 mb-2">Billing & Usage</h1>
        <p className="text-slate-400">Manage your subscription and monitor usage</p>
      </div>

      {/* Current Plan Overview */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-[#2e333d] rounded-xl p-5 border border-cyan-500/30">
        ...
        <div className="bg-[#151820] rounded-xl p-5 border border-slate-800/60">
          <div className="text-sm text-slate-400 mb-1">Monthly Spend</div>
          <div className="text-2xl font-bold text-slate-100">${mockBillingData.monthlySpend}</div>
          <div className="text-xs text-slate-500 mt-1">Billed monthly</div>
        </div>
        <div className="bg-[#151820] rounded-xl p-5 border border-slate-800/60">
          <div className="text-sm text-slate-400 mb-1">Token Usage</div>
          <div className="text-2xl font-bold text-slate-100">{mockBillingData.tokenUsage.percentage.toFixed(1)}%</div>
        ...
        <div className="bg-[#151820] rounded-xl p-5 border border-slate-800/60">
          ...
          <div className="text-2xl font-bold text-slate-100">{mockBillingData.prReviews.used}</div>
          <div className="text-xs text-slate-500 mt-1">of {mockBillingData.prReviews.limit} this month</div>
        </div>
      </div>

      {/* Usage Details */}
      <div className="bg-[#151820] rounded-xl p-6 border border-slate-800/60 mb-6">
        <h3 className="text-slate-100 font-medium text-lg mb-4">Usage This Month</h3>
        <div className="space-y-4">
          ...
          <div>
            ...
            {mockBillingData.tokenUsage.percentage >= 80 && (
              <p className="text-xs text-amber-400 mt-1">⚠️ Approaching limit - consider upgrading</p>
            )}
          </div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-slate-100 font-medium text-lg">Plans</h3>
          <div className="flex items-center gap-2 bg-[#2e333d] rounded-lg p-1 border border-slate-700/50">
            <button
              onClick={() => setBillingCycle('monthly')}
              className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
                billingCycle === 'monthly' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Monthly
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
                billingCycle === 'yearly' ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`bg-[#2e333d] rounded-xl p-5 border relative ${
                plan.current ? 'border-cyan-500/40' : 'border-slate-700/50'
              }`}
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-cyan-500/20 border border-cyan-500/30 rounded-full text-xs font-medium text-cyan-400">
                  Most Popular
                </div>
              )}
              {plan.current && (
                <div className="absolute -top-3 right-4 px-3 py-1 bg-emerald-500/20 border border-emerald-500/30 rounded-full text-xs font-medium text-emerald-400">
                  Current
                </div>
              )}
              <h4 className="text-slate-100 font-medium text-lg mb-2">{plan.name}</h4>
              <div className="mb-4">
              ...
              {plan.current ? (
                <button
                  ...
                  className="w-full py-2 bg-slate-800/60 text-slate-400 rounded-lg text-sm font-medium cursor-default border border-slate-700/50"
                >
                  Current Plan
                </button>
              ) : (
                <button
                  onClick={() => showComingSoon()}
                  className="w-full py-2 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 rounded-lg text-sm font-medium transition-colors border border-cyan-500/30"
                >
      ...
      {/* Invoice History */}
      <div className="bg-[#2e333d] rounded-xl p-6 border border-slate-700/50 mb-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-slate-100 font-medium text-lg">Invoice History</h3>
          <button
            onClick={() => showComingSoon()}
          ...
          <table className="w-full">
            ...
            <tbody>
              {mockInvoices.map((invoice) => (
                <tr key={invoice.id} className="border-b border-slate-800/40">
      ...
      {/* Payment Method */}
      <div className="bg-[#2e333d] rounded-xl p-6 border border-slate-700/50">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-slate-100 font-medium text-lg">Payment Method</h3>
          <button
            onClick={() => showComingSoon()}
        ...
        <div className="flex items-center gap-4 p-4 rounded-lg bg-slate-800/40 border border-slate-700/40">
          ...
          <span className="ml-auto px-2 py-1 rounded text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            Default
          </span>
        </div>
      </div>
