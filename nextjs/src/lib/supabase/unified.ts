...
export class SassClient {
    ...
    async getMyTodoList(page: number = 1, pageSize: number = 100, order: string = 'created_at', done: boolean | null = false) {
        ...
        let query = (this.client as any).from('todo_list').select('*').range(page * pageSize - pageSize, page * pageSize - 1).order(order)
        if (done !== null) {
    ...
    async createTask(row: any) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return (this.client as any).from('todo_list').insert(row)
    }

    async removeTask (id: number) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return (this.client as any).from('todo_list').delete().eq('id', id)
    }

    async updateAsDone (id: number) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return (this.client as any).from('todo_list').update({done: true}).eq('id', id)
    }
